...
* **الحالة**: **مكتملة ومؤمنة.**
* **التفاصيل**: تم توحيد إنهاء العمليات وتصريف الاتصالات عبر سجل مركزي لإيقاف التشغيل في `src/lib/services/shutdown.ts`. عند استلام إشارات الإيقاف (SIGTERM/SIGINT)، يتم إغلاق سيرفرات الـ WebSockets، وتصريف قوائم BullMQ، وإغلاق اتصالات Prisma و Redis بشكل آمن لمنع فقدان البيانات أو بقاء طلبات معلقة.
...
* **الدعوات (Invitations):** إضافة مستخدمين جدد تتم استحالة القيام بها إلا عبر مديري المنظمات بإرسال دعوات بريدية أو عبر توثيق الحسابات مع Google / Microsoft الموثقة للمؤسسة.
