 ...
 * وحدة المصادقة الثنائية
 *
 * Handles TOTP secret generation, verification, backup codes,
 * and 2FA lifecycle management.
 *
...
export async function disableTwoFactor(userId: string, password: string): Promise<AuthResponse> {
  try {
    ...
    // Verify password
    const isValid = await import('./password').then(m => m.verifyPassword)(password, user.password);
    if (!isValid) {
      return {
        success: false,
...
// ============================================
// Facade-compatible aliases
// These match the function names expected by auth-service.ts
// ============================================

/** Alias for facade compatibility — generate2FASecret(userId, email?) */
export async function generate2FASecret(userId: string, _email?: string) {
  return generateTwoFactorSecret(userId);
}

/** Alias for facade compatibility — enable2FA(userId, token) */
...
/** Alias for facade compatibility — verify2FA(userId, token) */
export async function verify2FA(userId: string, token: string) {
  return verifyTwoFactorCode(userId, token);
}

/** Alias for facade compatibility — check2FAStatus(userId) */
