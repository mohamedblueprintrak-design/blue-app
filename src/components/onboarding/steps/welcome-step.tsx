...
export default function WelcomeStep({ onNext, onSkip }: WelcomeStepProps) {
  ...
  return (
    <div className="flex flex-col items-center text-center px-4 sm:px-8 py-6">
      {/* Logo & Welcome */}
      <div className="mb-8">
        <LogoImage size={72} className="mx-auto mb-6 shadow-lg shadow-teal-500/20 rounded-2xl" />
      ...
      <div className="w-full max-w-md space-y-3 mb-8">
        ...
        {features.map((feature, i) => {
          ...
          return (
            <div
              key={i}
              className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-colors"
            >
      ...
      <div className="w-full max-w-md flex flex-col gap-2">
        <button
          onClick={onNext}
          className="w-full h-11 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 text-white font-semibold text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
