                ...
                {whatsappContacts.map(contact => (
                  <button
                    ...
                    className={cn('w-full flex items-center gap-3 p-3 rounded-lg transition-colors text-right',
                      selectedWhatsappContact === contact.phone ? 'bg-brand-navy-50 dark:bg-brand-navy-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'
                    )}
            ...
            <div className="px-3 py-2 border-t">
              <div className="flex gap-1.5 overflow-x-auto pb-2">
                {WHATSAPP_TEMPLATES.map((template, i) => (
                  <button
                    key={i}
                    className="flex-shrink-0 text-[10px] px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-brand-navy-50 hover:text-brand-navy-700 dark:hover:bg-brand-navy-900/20 dark:hover:text-brand-navy-400 transition-colors whitespace-nowrap"
