      ...
      {isLoading ? (
        ...
        projects.map((project: ProjectRow) => {
          ...
          return (
            <div
              key={project.id}
              className="rounded-xl border border-slate-200/70 dark:border-slate-700/40 bg-white dark:bg-slate-900/80 backdrop-blur-sm overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:shadow-xl hover:shadow-brand-navy-500/5 hover:-translate-y-1 hover:border-brand-navy-200 dark:hover:border-brand-navy-800/60 group"
              onClick={() => onRowClick(project.id)}
            >
              {/* Gradient header */}
              <div className="bg-gradient-to-l from-brand-navy-500/10 via-cyan-500/5 to-transparent dark:from-brand-navy-500/5 dark:via-cyan-500/5 dark:to-transparent px-4 pt-4 pb-2">
              ...
              <div className="px-4 pb-4 pt-2">
                ...
                {project.plotNumber && (
                  <div className="text-[10px] text-brand-navy-600 dark:text-brand-navy-400 font-medium mb-3 flex items-center gap-1">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex items-end gap-[2px] h-4">
                    {sparkline.map((h, i) => (
                      <div key={i} className={cn("w-[3px] rounded-full", h >= 75 ? "bg-brand-navy-400" : h >= 40 ? "bg-brand-navy-300" : "bg-amber-400")} style={{ height: `${Math.min(h, 100) * 0.16}px` }} />
                    ))}
                  </div>
