          ...
          <>
            ...
            <div className="w-full h-48 rounded-xl bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center gap-2">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500">
                {tAuto('auto.drawingPreview')}
              </span>
              <span className="text-[10px] text-slate-300 dark:text-slate-600">
                {drawing.filePath
                  ? (tAuto('auto.file1')) + drawing.filePath
                  : (tAuto('auto.noFileUploadedYet'))
                }
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-1">{tAuto('auto.status1')}</p>
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-1">{tAuto('auto.version')}</p>
                <p className="text-sm font-bold font-mono text-slate-900 dark:text-white">V{drawing.version}</p>
              </div>
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-1">{tAuto('auto.clash')}</p>
                <div className="flex items-center gap-1.5">
                  <span className={cn("w-2 h-2 rounded-full", drawing.clashDetected ? "bg-red-500" : "bg-emerald-500")} />
                  <span className="text-xs text-slate-700 dark:text-slate-300">
                    {drawing.clashDetected ? (tAuto('auto.detected')) : (tAuto('auto.none'))}
                  </span>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-1">{tAuto('auto.reviewer')}</p>
